 ...
 */
if (getenv('MB_FIXER_ACTIVE')) {
  add_action('rest_api_init', function () {
    register_rest_route('mb-qa/v1', '/ping', [
      'methods' => 'GET',
      'permission_callback' => '__return_true',
      'callback' => function () {
        return ['success' => true, 'ok' => true, 'ts' => time(), 'src' => 'root'];
      },
    ]);
  });
  function _mb_qa_echo() {
    wp_send_json(['success' => true, 'ok' => true, 'ts' => time(), 'src' => 'root']);
  }
  add_action('wp_ajax_mb_qa_echo', '_mb_qa_echo', 0);
  add_action('wp_ajax_nopriv_mb_qa_echo', '_mb_qa_echo', 0);
  add_action('wp_head', function () {
    echo "<!-- MB_FIXER_ACTIVE -->\n";
    echo "<div id=\"mb-fixer-active\" style=\"display:none\">MB_FIXER_ACTIVE</div>\n";
  });
...
echo "::endgroup::"
echo "== inject MU plugin proactively (markers + REST + AJAX) =="
MU_PHP='<?php
...
if [ "$code" != "200" ] || ! has_ok_or_success_true ajax.json; then
  echo "Inject MU in runtime and retry (resolve WPMU_PLUGIN_DIR via wp-load; fallback to candidates)"
  MU_PHP='<?php
add_action("rest_api_init", function(){ register_rest_route("mb-qa/v1","/ping",[
  "methods"=>"GET","permission_callback"=>"__return_true",
  "callback"=>function(){ return ["success"=>true,"ok"=>true,"ts"=>time(),"src"=>"mu"]; }
